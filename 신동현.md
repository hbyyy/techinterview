...
&emsp;1. 브라우저 혹은 OS의 캐시 메모리 확인  
&emsp;2. Resolver Server(ISP)에 쿼리  
&emsp;3. Root Server에 쿼리 -> 경로 알려줌 (전 세계 13개 세트의 루트 서버 존재, 12개의 기관에 의해 운영)  
&emsp;4. TLD(Top-level-domain) 서버로 보내줌 (.com .net 과 같은 도메인을 관리)  
&emsp;5. Authoritative name server가 도메인에 관한 모든 정보를 보관 (IP 주소 포함)  
&emsp;6. ISP가 해당 정보를 브라우저에게 알려주고, 자신의 서버에도 저장  
