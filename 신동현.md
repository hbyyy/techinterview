...
#### 추가 내용
&emsp;-많은 웹사이트들이 데이터의 종류에 상관없이 HTTPS를 기본으로 사용  
&emsp;&emsp;-> SSL을 사용하지 않으면 크롬에 ‘안전하지 않은 웹사이트’로 표시되고 서치 랭킹에도 악영향  

&emsp;-CA 인증서 발급 방식  
1. 사이트의 정보와 공개키를 인증 기관(CA)에 제출
2. 인증 기관은 검증 후, 사이트 정보와 공개키를 CA 개인키로 암호화하여 인증서 발급
3. 인증 기관은 웹 브라우저에게 자신의 공개키를 제공
4. 사용자가 웹 브라우저로 사이트에 접속하면 인증서 수신
5. 브라우저가 CA 공개키를 이용해서 인증서를 해독하고 검증
6. 해독해서 얻은 사이트의 공개키로 데이터를 암호화하여 전송
